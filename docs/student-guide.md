...
```bash
git add .
git commit -m "Implement menu-service health endpoint"
git push
```
...
| 6 | Use service names, not `localhost` | Use `http://menu-service:5000` not `http://localhost:5001` |
...
| One service cannot call another | Using `localhost` instead of service name | Change to `http://order-service:5000` (use compose service name + internal port) |
